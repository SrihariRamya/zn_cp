...
function VideoContextProperty(properties) {
  const { contextProperties, editorContext, setEditorContext } = properties;
      ...
      map(editorContext, (item) => {
        map(item.column, (element) => {
          if (
            ...
            element.section.section_video_content = undefined;
            element.section.preview = undefined;
          }
          return element;
        });
        return item;
